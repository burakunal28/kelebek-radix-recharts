		...
		return (
			...
			>
				{state.segments.map((segment, i) => (
					<TimeSegment
						key={`${segment.type}-${i}-${segment.text}`}
						segment={segment}
						state={state}
					/>
				))}
...
const PresetContainer = <TPreset extends Preset, TValue>({
	...
	const isDateRangePresets = (preset: TPreset): boolean => {
		return "dateRange" in preset && !("date" in preset);
	};
	const isDatePresets = (preset: TPreset): boolean => {
		return "date" in preset && !("dateRange" in preset);
	};

	const handleClick = (preset: TPreset) => {
		if (isDateRangePresets(preset)) {
			onSelect((preset as unknown as DateRangePreset).dateRange as TValue);
		} else if (isDatePresets(preset)) {
			onSelect((preset as unknown as DatePreset).date as TValue);
		}
	};
	...
	const matchesCurrent = (preset: TPreset) => {
		if (isDateRangePresets(preset)) {
			...
			return (
				value &&
				compareRanges(value, (preset as unknown as DateRangePreset).dateRange)
			);
		} else if (isDatePresets(preset)) {
			const value = currentValue as Date | undefined;

			return (
				value && compareDates(value, (preset as unknown as DatePreset).date)
			);
		}
...
const SingleDatePicker = ({
	...
	const getInitialTime = () => {
		...
		if (defaultValue) {
			return new Time(defaultValue.getHours(), defaultValue.getMinutes());
		}
		return new Time(0, 0);
	};

	const [time, setTime] = React.useState<TimeValue | null>(getInitialTime());
...
const RangeDatePicker = ({
	...
	const getInitialStartTime = React.useCallback(() => {
		if (value?.from) {
			return new Time(value.from.getHours(), value.from.getMinutes());
		}
	...
	const getInitialEndTime = React.useCallback(() => {
		if (value?.to) {
			return new Time(value.to.getHours(), value.to.getMinutes());
		}
		if (defaultValue?.to) {
			return new Time(defaultValue.to.getHours(), defaultValue.to.getMinutes());
		}
		return new Time(0, 0);
	...
	const onCancel = () => {
		...
		const startTimeValue = initialRange?.from
			? new Time(initialRange.from.getHours(), initialRange.from.getMinutes())
			: new Time(0, 0);
		setStartTime(startTimeValue);

		const endTimeValue = initialRange?.to
	...
	React.useEffect(() => {
		...
		setEndTime(getInitialEndTime());
	}, [value, defaultValue, getInitialStartTime, getInitialEndTime]);

	const displayRange = React.useMemo(() => {
		if (!range) return null;

		const fromStr = range.from
			? formatDate(range.from, locale, showTimePicker)
			: "";
		const toStr = range.to ? formatDate(range.to, locale, showTimePicker) : "";
...
const validateDatePreset = (preset: DatePreset, rules: PickerProps) => {
...
const validatePresets = (
	...
	presets.forEach((preset) => {
		if ("date" in preset) {
			validateDatePreset(preset, rules);
		} else if ("dateRange" in preset) {
			validateDateRangePreset(preset, rules);
		}
	});
};
