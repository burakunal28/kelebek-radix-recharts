...
export function ChartCard({
	...
	const chartData = allDatesInInterval
		?.map((date, index) => {
			...
			return {
				...
				previousFormattedDate: prevOverview
					...
					: undefined,
				previousValue:
					selectedPeriod !== "no-comparison" ? previousValue || 0 : 0,
				evolution:
		...
		.sort(
			(a, b) =>
				new Date(a.formattedDate.split("/").reverse().join("-")).getTime() -
				new Date(b.formattedDate.split("/").reverse().join("-")).getTime(),
		);
			...
			<div className="mt-2 flex items-baseline justify-between">
				...
				{selectedPeriod !== "no-comparison" && (
					<dd className="text-sm text-gray-500">
						önceki {formatter(previousValue)}
					</dd>
				)}
			</div>
			<LineChart
